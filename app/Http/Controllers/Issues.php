...
class Issues extends Controller
{
    ...
    public function index() {
    	$Pages = new Page;
		$posts = $Pages->setPerPage($this->perPage)->setType(4)->showFrontPages();
		$data['news'] = $posts;
		$data['postTerkini'] = $Pages->setType(4)->setLimit(10)->getLatest();
		$data['tag'] = explode(",",app('keywords'));
		$data['title'] = trans('routes.issues');
		$data['keywords'] = trans('routes.issuesKeywords');
		$data['description'] = trans('routes.issuesInformation');
